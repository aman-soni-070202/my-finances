...
CREATE TABLE `__new_transactions` (
	...
	FOREIGN KEY (`category_name`,`category_type`) REFERENCES `categories`(`name`,`type`) ON UPDATE no action ON DELETE no action,
	CONSTRAINT "transaction_type_check" CHECK(type IN ('income', 'expense')),
	CONSTRAINT "is_card_check" CHECK(isCard IN (0, 1)),
	CONSTRAINT "payment_method_type_check" CHECK((isCard = 1 AND paymentMethodType = 'creditCard') OR (isCard = 0 AND paymentMethodType = 'bankAccount'))
);
--> statement-breakpoint
INSERT INTO `__new_transactions`("id", "date", "amount", "note", "category_name", "category_type", "type", "paymentMethodId", "paymentMethodType", "isCard") SELECT "id", "date", "amount", "note", "category_name", "category_type", "type", "paymentMethodId", "paymentMethodType", "isCard" FROM `transactions`;--> statement-breakpoint
